...
- **状态**：运行状态、操作统计、下次检查时间
- **设置**：玩家等级、种植策略（自动最优/手动指定）、功能开关、检查间隔、游戏路径
...
│   ├── window_manager.py       # 窗口定位与管理（支持自动启动游戏）
...
3. 窗口标题需包含"QQ 经典农场"，可在设置中修改窗口关键词

**Q: 模板匹配不准确？**
模板需要在自己的设备上采集。不同分辨率、DPI 缩放会导致模板不匹配。程序内置了多尺度匹配（0.8x~1.2x），但差异过大时仍需重新采集。
