...
> **提示：** 目前测试使用的是迎春花种子，`shop_迎春花.png` 模板已通过导入工具生成。其他种子的商店模板因显示样式差异较大，建议自己在商店页面重新采集，以确保匹配准确率。

> **注意：** 普通土地模板（`land_empty.png`、`land_empty2.png`）需要手动采集。导入工具仅提供种子图片，不包含土地模板。请在农场主页框选空地进行采集，采集时注意文件名不要与本地已有的模板冲突。
...
- **设置**：
  ...
  - **策略**：自动最优（推荐）/ 手动指定作物
  - **功能开关**：
    - 推荐开启：收获 ☑、播种 ☑、**施肥 ☑**、浇水 ☑、除草 ☑、除虫 ☑
    - 其余功能：出售、偷菜、帮忙、任务、扩建（本人未测试，不清楚原仓库实现情况）
  - **买种**：勾选后种子不足时自动去商店购买（需提前采集对应种子的 `shop_` 模板）；取消勾选需手动购买足够种子
  - **检查间隔**：农场检查间隔（默认 1 分钟）、好友检查间隔（默认 30 分钟）
  - **游戏路径**：选择 QQ 农场快捷方式，找不到窗口时自动启动
...
`setup.bat` 需要使用 ANSI/GBK 编码。如果出现乱码，请：
1. 用记事本打开 `setup.bat`
2. 文件 → 另存为 → 编码选择 ANSI
3. 或者直接使用手动安装方式
